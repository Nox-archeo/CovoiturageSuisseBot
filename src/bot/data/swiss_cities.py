...
def load_localities() -> Dict:
    ...
    try:
        # Essayer d'abord swiss_localities.json
        file_path = Path(__file__).parent.parent / 'data' / 'swiss_localities.json'
        logger.info(f"Trying to load localities from {file_path}")
        
        if not file_path.exists():
            # Si le fichier n'existe pas, essayer cities.json
            file_path = Path(__file__).parent.parent / 'src' / 'bot' / 'data' / 'cities.json'
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if 'cities' in data:
                # Format cities.json : convertir en dictionnaire
                cities_dict = {}
                for city in data['cities']:
                    city_key = city['name']
                    # Adapter le format si nécessaire (npa → zip)
                    city_data = {
                        'name': city['name'],
                        'canton': city['canton'],
                        'zip': city.get('zip', city.get('npa', ''))
                    }
                    cities_dict[city_key] = city_data
                logger.info(f"Loaded {len(cities_dict)} localities from cities.json format")
                return cities_dict
            else:
                # Format direct
                logger.info(f"Loaded {len(data)} localities from direct format")
                return data
                
    except FileNotFoundError as e:
        logger.error(f"Localities file not found: {e}")
        return {}
